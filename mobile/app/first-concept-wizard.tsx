...
import { firstConceptTemplates, type FirstConceptTemplate } from "@/lib/concept-templates";
import { createCustomTemplate, CUSTOM_TEMPLATES_KEY, parseCustomTemplates } from "@/lib/custom-templates";
import { conceptKinds, relationshipTypes, type ConceptKind, type RelationshipType } from "@/lib/knowledge-data";
import { FIRST_CONCEPT_WIZARD_KEY, previousWizardStep, validateWizardNote, validateWizardTitle, wizardCompletionStatus } from "@/lib/first-concept-wizard-state";
...
export default function FirstConceptWizardScreen() {
  ...
  const [customTemplates, setCustomTemplates] = useState<FirstConceptTemplate[]>([]);
  const [templateMessage, setTemplateMessage] = useState("");

  const availableTargets = useMemo(() => concepts.filter((concept) => concept.id !== createdConceptId), [concepts, createdConceptId]);
  const templates = useMemo(() => [...firstConceptTemplates, ...customTemplates], [customTemplates]);
  const currentStepLabel = step === 1 ? "Name the idea" : step === 2 ? "Give it context" : "Make one connection";
  const progress = `${step} of 3`;

  useEffect(() => {
    AsyncStorage.getItem(CUSTOM_TEMPLATES_KEY).then((stored) => setCustomTemplates(parseCustomTemplates(stored))).catch(() => undefined);
  }, []);
  ...
  const createFirstConcept = () => {
    ...
    router.replace({ pathname: "/first-relationship-wizard", params: { id: concept.id } });
  };

  const saveCurrentAsTemplate = async () => {
    const titleError = validateWizardTitle(title);
    const noteError = validateWizardNote(note);
    if (titleError || noteError) { setError(titleError ?? noteError ?? ""); return; }
    const template = createCustomTemplate({ label: title.trim(), title: title.trim(), kind, note: note.trim() }, [...templates]);
  ...
  return <KeyboardAvoidingView style={styles.screen} behavior={Platform.OS === "ios" ? "padding" : undefined}>
    ...
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      ...
      {step === 1 ? <StepOne title={title} setTitle={setTitle} kind={kind} setKind={setKind} selectedTemplate={selectedTemplate} templates={templates} onApplyTemplate={applyTemplate} /> : null}
      {step === 2 ? <StepTwo title={title} note={note} setNote={setNote} onSaveTemplate={saveCurrentAsTemplate} templateMessage={templateMessage} /> : null}
