...
export default function TodayScreen() {
  ...
  const [exportRecoveryHint, setExportRecoveryHint] = useState("");
  ...
  const exportLocalGraph = useCallback(async () => {
    ...
    if (isProtectedExport) {
      ...
      const hintValidation = validateExportRecoveryHint(exportRecoveryHint, exportPassphrase);
      if (!hintValidation.valid) { setExportState("error"); setExportMessage(hintValidation.message); return; }
    ...
    try {
      ...
      const bundle = isProtectedExport ? await buildPasswordProtectedExportBundle(plainBundle, homeLocalExportBundleFilename(exportedAt), exportPassphrase, exportRecoveryHint) : plainBundle;
      if (Platform.OS === "web") {
      ...
      const nextStatus = await persistLocalExport({ format: isProtectedExport ? "protected-zip" : "complete-zip", filename: bundleFilename, conceptCount: allConcepts.length, connectionCount: allConnections.length });
      setExportStatus(nextStatus);
      setExportPassphrase("");
      setExportPassphraseConfirmation("");
      setExportRecoveryHint("");
  ...
  }, [allConcepts, allConnections, exportPassphrase, exportPassphraseConfirmation, exportRecoveryHint, exportState, isProtectedExport]);
  ...
  const toggleExportProtection = useCallback(() => {
    setIsProtectedExport((current) => {
      const next = !current;
      if (!next) { setExportPassphrase(""); setExportPassphraseConfirmation(""); setExportRecoveryHint(""); }
      return next;
    });
  }, []);
  const toggleRestoreTestReminders = useCallback(() => {
    setExportStatus((current) => {
      const next = { ...current, restoreTestRemindersEnabled: !current.restoreTestRemindersEnabled };
      void saveLocalExportStatus(next);
      return next;
    });
  }, []);
  const markRestoreTestComplete = useCallback(() => {
    setExportStatus((current) => {
      const next = recordLocalRestoreTest(current);
      void saveLocalExportStatus(next);
      return next;
    });
  }, []);
...
function PopulatedDashboard({ concepts, relationships, backupSummary, onQuickRun, quickRunState, quickRunMessage, quickRunProgress, onExport, exportState, exportMessage, exportStatus, onOpenHistory, onOpenRecoveryTools, onToggleExportReminders, onToggleRestoreTestReminders, onMarkRestoreTestComplete, isProtectedExport, exportPassphrase, exportPassphraseConfirmation, exportRecoveryHint, onToggleExportProtection, onChangeExportPassphrase, onChangeExportPassphraseConfirmation, onChangeExportRecoveryHint }: { concepts: number; relationships: number; backupSummary: BackupStatusSummary | null; onQuickRun: () => void; quickRunState: "idle" | "running"; quickRunMessage: string; quickRunProgress: SyncProgressState; onExport: () => void; exportState: HomeLocalExportState; exportMessage: string; exportStatus: LocalExportStatus; onOpenHistory: () => void; onOpenRecoveryTools: () => void; onToggleExportReminders: () => void; onToggleRestoreTestReminders: () => void; onMarkRestoreTestComplete: () => void; isProtectedExport: boolean; exportPassphrase: string; exportPassphraseConfirmation: string; exportRecoveryHint: string; onToggleExportProtection: () => void; onChangeExportPassphrase: (value: string) => void; onChangeExportPassphraseConfirmation: (value: string) => void; onChangeExportRecoveryHint: (value: string) => void }) {
  ...
  const checklistSteps = firstExportChecklist(concepts, relationships, isProtectedExport);
  const backupHealth = summarizeLocalBackupHealth(exportStatus);
  const restoreTestDue = shouldShowRestoreTestReminder(exportStatus);
  return <View>
...
function FirstExportChecklist({ steps, isProtectedExport, onEnableProtection, onExport, exportState }: { steps: FirstExportChecklistStep[]; isProtectedExport: boolean; onEnableProtection: () => void; onExport: () => void; exportState: HomeLocalExportState }) { return <View style={styles.firstExportCard}><Text style={styles.firstExportEyebrow}>FIRST LOCAL EXPORT</Text><Text style={styles.firstExportTitle}>Keep your first graph copy simple.</Text><Text style={styles.firstExportDetail}>Follow these short steps, then save the ZIP through your device. You can add protection now or export a standard local copy.</Text>{steps.map((step, index) => <View key={step.id} style={styles.firstExportStep}><View style={[styles.firstExportStepMark, step.complete && styles.firstExportStepMarkComplete]}><Text style={styles.firstExportStepNumber}>{step.complete ? "✓" : index + 1}</Text></View><View style={styles.firstExportStepCopy}><Text style={styles.firstExportStepTitle}>{step.title}</Text><Text style={styles.firstExportStepDetail}>{step.detail}</Text>{step.id === "protection" && !isProtectedExport ? <Pressable accessibilityRole="button" accessibilityLabel="Turn on passphrase protection for the first export" onPress={onEnableProtection} style={({ pressed }) => [styles.firstExportAction, pressed && styles.pressed]}><Text style={styles.firstExportActionText}>{step.actionLabel}</Text></Pressable> : null}{step.id === "export" && step.actionLabel ? <Pressable accessibilityRole="button" accessibilityLabel="Create first graph export" disabled={exportState === "exporting"} onPress={onExport} style={({ pressed }) => [styles.firstExportAction, styles.firstExportActionPrimary, pressed && styles.pressed, exportState === "exporting" && styles.disabled]}><Text style={styles.firstExportActionText}>{exportState === "exporting" ? "Preparing…" : step.actionLabel}</Text></Pressable> : null}</View></View>)}</View>; }
...
const styles = StyleSheet.create({
  ...
  firstExportCard: { marginTop: 14, borderRadius: 18, padding: 14, backgroundColor: "#20264D", borderWidth: 1, borderColor: "#665FC0" },
  firstExportEyebrow: { color: "#BDB7FF", fontSize: 9, letterSpacing: 1.05, fontWeight: "900" },
  firstExportTitle: { color: "#F2F1FF", fontSize: 16, fontWeight: "900", marginTop: 5 },
  firstExportDetail: { color: "#C2C1DF", fontSize: 11, lineHeight: 16, marginTop: 5 },
  firstExportStep: { flexDirection: "row", marginTop: 13 },
  firstExportStepMark: { width: 25, height: 25, borderRadius: 13, backgroundColor: "#353C6B", borderWidth: 1, borderColor: "#7371C5", alignItems: "center", justifyContent: "center", marginRight: 10 },
  firstExportStepMarkComplete: { backgroundColor: "#326270", borderColor: "#70CFD2" },
  firstExportStepNumber: { color: "#F1EFFF", fontSize: 11, fontWeight: "900" },
  firstExportStepCopy: { flex: 1 },
  firstExportStepTitle: { color: "#F1F0FE", fontSize: 12, fontWeight: "900" },
  firstExportStepDetail: { color: "#B8B6D6", fontSize: 10, lineHeight: 14, marginTop: 2 },
  firstExportAction: { alignSelf: "flex-start", minHeight: 32, borderRadius: 9, paddingHorizontal: 10, justifyContent: "center", backgroundColor: "#3A427A", borderWidth: 1, borderColor: "#8581DF", marginTop: 7 },
  firstExportActionPrimary: { backgroundColor: "#605AB9", borderColor: "#AAA4FF" },
  firstExportActionText: { color: "#F5F4FF", fontSize: 10, fontWeight: "900" },
  backupConfidenceCard: { marginTop: 14, borderRadius: 18, padding: 14, backgroundColor: "#17263A", borderWidth: 1, borderColor: "#3A5E7A" },
  backupConfidenceHeader: { flexDirection: "row", alignItems: "flex-start" },
  backupConfidenceDot: { width: 9, height: 9, borderRadius: 5, marginTop: 4, marginRight: 9 },
  backupConfidenceCopy: { flex: 1 },
  backupConfidenceEyebrow: { color: "#87D7E4", fontSize: 9, letterSpacing: 1.05, fontWeight: "900" },
  backupConfidenceTitle: { color: "#E6F6FA", fontSize: 14, fontWeight: "900", marginTop: 4 },
  backupConfidenceDetail: { color: "#A9C8D4", fontSize: 10, lineHeight: 15, marginTop: 3 },
  backupConfidenceAction: { minHeight: 34, borderRadius: 9, marginTop: 10, justifyContent: "center", alignItems: "center", backgroundColor: "#27566A" },
  backupConfidenceActionText: { color: "#E3FAFB", fontSize: 10, fontWeight: "900" },
  restoreTestPanel: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", borderTopWidth: 1, borderColor: "#31516A", marginTop: 11, paddingTop: 11 },
  restoreTestCopy: { flex: 1, paddingRight: 10 },
  restoreTestTitle: { color: "#D9EEF3", fontSize: 11, fontWeight: "900" },
  restoreTestDetail: { color: "#92B4C2", fontSize: 9, lineHeight: 13, marginTop: 2 },
  restoreTestToggle: { minWidth: 42, minHeight: 29, borderRadius: 8, backgroundColor: "#254B60", justifyContent: "center", alignItems: "center" },
  restoreTestToggleText: { color: "#B9EDF0", fontSize: 10, fontWeight: "900" },
  restoreTestActions: { flexDirection: "row", gap: 8, marginTop: 10 },
  restoreTestAction: { flex: 1, minHeight: 34, borderRadius: 9, borderWidth: 1, borderColor: "#4E7E91", justifyContent: "center", alignItems: "center", paddingHorizontal: 7 },
  restoreTestActionPrimary: { backgroundColor: "#336875", borderColor: "#6FBCC3" },
  restoreTestActionText: { color: "#DDF9FA", fontSize: 9, fontWeight: "900", textAlign: "center" },
  exportSuccessOverlay: { flex: 1, backgroundColor: "rgba(5, 11, 23, 0.76)", justifyContent: "flex-end", padding: 16 },
  exportSuccessSheet: { borderRadius: 24, padding: 20, backgroundColor: "#14243A", borderWidth: 1, borderColor: "#426B88" },
  ...
  exportSuccessPrimary: { minHeight: 44, borderRadius: 12, backgroundColor: "#3D8995", justifyContent: "center", alignItems: "center", marginTop: 18 },
  exportSuccessPrimaryText: { color: "#F2FEFE", fontSize: 12, fontWeight: "900" },
  exportSuccessDismiss: { minHeight: 38, justifyContent: "center", alignItems: "center", marginTop: 4 },
  exportSuccessDismissText: { color: "#B9D5DE", fontSize: 11, fontWeight: "800" },
  ...
  exportMetaRow: { marginTop: 9 },
  historyLink: { minHeight: 31, borderRadius: 9, paddingHorizontal: 9, backgroundColor: "#1B4651", flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  historyLinkText: { color: "#C9F3F4", fontSize: 10, fontWeight: "900" },
  historyLinkMeta: { color: "#86CFD6", fontSize: 9, fontWeight: "800" },
