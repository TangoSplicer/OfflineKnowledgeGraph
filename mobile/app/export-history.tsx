...
import { clearLocalExportHistory, emptyLocalExportStatus, filterLocalExportHistory, LOCAL_EXPORT_HISTORY_RETENTION_OPTIONS, loadLocalExportStatus, saveLocalExportStatus, setLocalExportHistoryRetention, sortLocalExportHistory, type LocalExportHistoryEntry, type LocalExportHistoryFilter, type LocalExportHistoryRetention, type LocalExportHistorySort, type LocalExportStatus } from "@/lib/local-export-status";
...
export default function ExportHistoryScreen() {
  ...
  const [sort, setSort] = useState<LocalExportHistorySort>("recent");
  ...
  const visibleHistory = useMemo(() => sortLocalExportHistory(filterLocalExportHistory(status.history, query, format), sort), [format, query, sort, status.history]);
...
const styles = StyleSheet.create({
  ...
  clearHistoryButton: { minHeight: 40, borderRadius: 11, paddingHorizontal: 12, marginTop: 12, flexDirection: "row", alignItems: "center", justifyContent: "space-between", backgroundColor: "#38243A", borderWidth: 1, borderColor: "#77475C" },
  clearHistoryButtonConfirm: { backgroundColor: "#63283D", borderColor: "#E36F91" },
  clearHistoryText: { color: "#FFDCE5", fontSize: 10, fontWeight: "900" },
  clearHistoryArrow: { color: "#FFB7C8", fontSize: 17 },
